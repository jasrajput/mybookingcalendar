...
import { createEvent, eventReset } from '../features/event/eventSlice'
...
const NewEvent = () => {
  ...
  const { eventError, eventMessage, eventLoading, eventSuccess } = useSelector(store => store.event)
  ...
  const submitHandler = e => {
    ...
    dispatch(createEvent({ meetingHours: meetingTime, name: eventName, description: eventDescription, location: ((meetingLocation === 'Location') ? 'Google meet' : meetingLocation), duration: ((duration === 'Duration') ? '30 min' : duration), daysAdvance: daysAdvance, onConfirmation, redirectURL, eventType, team: (eventType === 'Team Event') ? filteredTeams[0]._id : '', timeZone: user.homeTimeZone }))
  }
        ...
        <div className={`loginForm_container ${styles.newEventFormContainer}`}>
          ...
          <form onSubmit={submitHandler}>
            ...
            <div className={`p-relative`}>
              ...
              <input type="text" value={teamName} onChange={(e) => setTeamName(e.target.value)} style={{display: `${eventType === 'Team Event' ? 'block' : 'none'}`}} placeholder="Enter Team Name"/>
            ...
            <input id="flashBtn" className={`${styles.eventSubmitBtn}`} type="submit" value={eventLoading ? 'Loading...' : "Create Event"} disabled={eventLoading ? true : false}/>
          </form>
        </div>
      </section>
    </Layout>
