...
export default async (req, res) => {
    try {
        if(req.method === 'GET'){
            ...
            const backendRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/calendar/events?date=${req.query.date}&from=${req.query.from?.replace('+', '%2B')}&to=${req.query.to?.replace('+', '%2B')}`, {
                method: 'GET',
                headers: {
            ...
            if(backendRes.ok){
                ...
                res.status(200).json(backendData.events)
            }else{
